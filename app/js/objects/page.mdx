...
В JavaScript объекты изменяемы, и их можно модифицировать с помощью методов, таких как `Object.assign()` и `Object.freeze()`.

### `Object.assign()`

Метод `Object.assign()` копирует все перечислимые свойства из одного или нескольких объектов в целевой объект. Он также позволяет добавлять или обновлять свойства.
...
Метод возвращает целевой объект.

Пример:

```js
const person = { name: "Иван", age: 30 };
const address = { city: "Москва", country: "Россия" };

const updatedPerson = Object.assign(person, address);

console.log(updatedPerson); 
// { name: "Иван", age: 30, city: "Москва", country: "Россия" }
```
...
> [!NOTE]
> 
> **Пример использования с несколькими источниками**
> 
> ```js
> const obj1 = { a: 1 };
> const obj2 = { b: 2 };
...
Метод `Object.freeze()` замораживает объект, делая его неизменяемым (поверхностно).
...
- Замороженные объекты остаются неизменными, но вложенные объекты можно изменять.
...
person.age = 35;  // не будет выполнено
person.city = "Москва";  // не будет выполнено
...
> **Важный момент**
> 
> Вложенные объекты остаются изменяемыми после заморозки родительского объекта.
...
> 
> ```js
> const person = { name: "Иван", address: { city: "Москва", country: "Россия" } };
> 
> Object.freeze(person);
> 
> person.address.city = "Санкт-Петербург";  // Изменение возможно
> 
...
> deepFreeze(person);
> 
> person.address.city = "Санкт-Петербург";  // Не будет выполнено
...
Деструктуризация позволяет извлечь несколько свойств объекта в отдельные переменные с помощью синтаксиса:
```js
const { property1, property2 } = object;
```
- `property1`, `property2` - это имена свойств объекта.
- `object` - это сам объект, из которого мы хотим извлечь значения.

Пример:
```js
const person = { name: "Иван", age: 30, gender: "Мужской" };
...
> 
> Если имена переменных должны отличаться от свойств объекта, можно использовать двоеточие:
> ```js
> const { name: fullName, age: years } = person;
> 
> console.log(fullName);  // "Иван"
...
> **Дефолтные значения**
> 
> Для свойств, которые могут быть отсутствующими, можно задать значение по умолчанию:
> ```js
> const { name, age, gender = "Не указан" } = person;
> 
> console.log(gender);   // "Не указан" (значение по умолчанию)
> ```
...
> ```js
> const person = { name: "Иван", age: 30, address: { city: "Москва", country: "Россия" } };
> 
...
> [!NOTE]
> 
> **Остаточные параметры (Rest syntax)**
> 
> Можно собрать оставшиеся свойства в новый объект с помощью оператора `...`:
> ```js
> const { name, age, ...otherInfo } = person;
> 
> console.log(otherInfo); // { gender: "Мужской", country: "Россия" }
> ```
...
> 
> Деструктуризация упрощает работу с объектами в параметрах функций:
> ```js
...
> ```
> В этом примере мы деструктурируем объект прямо в параметрах функции `greet`, извлекая только свойства `name` и `age`.

> [!NOTE]
> 
> **Деструктуризация с массивами**
> 
> Можно использовать деструктуризацию объектов и массивов вместе для сложных структур:
> ```js
> const data = { id: 1, user: { name: "Иван", age: 30 }, tags: ["developer", "javascript"] };
> 
> // Деструктуризация с массивами и объектами
> const { id, user: { name, age }, tags: [tag1, tag2] } = data;
> 
> console.log(tag1);  // "developer"
> console.log(tag2);  // "javascript"
> ```
...
Метод `hasOwnProperty()` проверяет, является ли указанное свойство **собственным** (не унаследованным) для объекта.

```js
obj.hasOwnProperty(prop)
```
- `obj` - объект, у которого проверяется наличие свойства.
- `prop` - строка или символ, имя свойства.

Метод возвращает `true`, если свойство является собственным, и `false` - если нет.
...
> **Важный момент**
> 
> `hasOwnProperty()` не проверяет свойства, унаследованные от прототипа.

### `Object.keys()`
Метод `Object.keys()` возвращает массив строк, содержащий все собственные **перечисляемые свойства** объекта.

```js
Object.keys(obj)
```
- `obj` - объект для получения ключей.

Метод возвращает массив строк, где каждый элемент - это ключ одного из свойств объекта.

```js
const obj = { name: "Иван", age: 30, gender: "Мужской" };
...
Object.values(obj)
```
- `obj` - объект для получения значений свойств.

Метод возвращает массив значений свойств объекта.

```js
const obj = { name: "Иван", age: 30, gender: "Мужской" };
console.log(Object.values(obj));  // ["Иван", 30, "Мужской"]
```

### `Object.entries()`

Метод `Object.entries()` возвращает массив **пар ключ-значение** объекта в виде массивов из двух элементов.

```js
Object.entries(obj)
```
- `obj` - объект для получения пар ключ-значение.

Метод возвращает массив массивов, каждый из которых содержит пару `[ключ, значение]`.
